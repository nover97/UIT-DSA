...
string addBitStrings( string first, string second ){
    ...
    for (int i = length-1 ; i >= 0 ; i--){			// Cong tat ca cac bit tung bit mot
    ...
    if (carry)  result = '1' + result;				// Neu tran them '1' o phia truoc
    return result;	}
...
long int multiply(string X, string Y){
    ...
    if (n == 0) return 0;						// Dieu kien dung cua de quy
